...
from modeling import BertForSequenceClassification, BertForMultiLabelClassification
...
class DataProcessor(object):
    ...
    def _read_csv(cls, input_file, quotechar=None):
        """Reads a tab separated value file."""
        lines = pd.read_csv(input_file).values.tolist()
        return lines
...
class MrpcProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_csv(os.path.join("../dataset/toxic_train")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_csv(os.path.join('../dataset/toxic_eval')), "dev")

    def get_test_examples(self, data_dir):
        
        return self._create_examples(
            self._read_csv(os.path.join("../dataset/toxic_eval")), "test")
...
#                 continue
            ...
            if set_type == "test":
                ...
                label = [line[2], line[3], line[4], line[5], line[6], line[7]] 
            examples.append(
...
def accuracy(out, labels):
    pred = np.sum((np.array(out) > 0.5) == np.array(labels), axis=1)
    count = 0
    for i in pred:
        if i == 6:
            count += 1
...
#                 print('training acc is {}'.format(tmp_eval_accuracy))
                if n_gpu > 1:
    ...
    model = BertForMultiLabelClassification.from_pretrained(args.bert_model, state_dict=model_state_dict, num_labels=num_labels)
    model.to(device)
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        pred_features = convert_examples_to_features(
            pred_examples, None, args.max_seq_length, tokenizer)
        logger.info("***** Running evaluation *****")
